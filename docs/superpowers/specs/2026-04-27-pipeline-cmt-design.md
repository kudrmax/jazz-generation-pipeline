...
- Наш pipeline-фактор → adapter (через `CMTPipelineConfig`): `seed_strategy`, `prime_bars`, `custom_pkl_path`.
...
| `custom_pkl` | Читает `config.custom_pkl_path` (формат CMT instance pkl: `{'pitch': ndarray, 'rhythm': ndarray, 'chord': csc_matrix}`). Берёт `pitch[:prime_len]`, `rhythm[:prime_len]`. `chord` из pkl игнорируется — используется наш chord_chroma из progression. |
...
class CMTAdapter(ModelAdapter):
    ...
    def prepare(self, progression: ChordProgression, tmp_dir: Path) -> dict:
        ...
        # 2. Валидация:
        #    - seed_strategy ∈ {tonic_held, tonic_quarters, custom_pkl}
        #    - custom_pkl ⇒ custom_pkl_path задан
...
| `test_cmt_seed` | `build_seed` с явным `frame_per_bar` и `prime_len`: все 3 стратегии; `tonic_held` для разных первых аккордов даёт разный prime_pitch[0]; `tonic_quarters` ставит онсеты на правильных позициях для разных fpb/bpb; `custom_pkl` берёт из pkl, валит ValueError если короче prime_len |
| `test_cmt_validation` | `prepare` валит ValueError для: неверного seed_strategy; custom_pkl без path; prime_bars < 1 или > num_bars; total_frames в progression ≠ max_len; frame_per_bar % beats_per_bar ≠ 0. Использует **fake hparams.yaml** в tmp_path — реальные веса для тестов не нужны |
...
| # | Риск | Митигация |
|---|---|---|
| 1 | Точные значения `SUSTAIN_PITCH_IDX` / `ONSET_RHYTHM_IDX` / `SUSTAIN_RHYTHM_IDX` определяются на стадии реализации Task 7 — нужно открыть локальный `seed_instance.pkl` и подтвердить. Спека эти числа не фиксирует, чтобы не вводить в заблуждение | В Task 7 первый шаг — diagnostic-скрипт: открыть pkl, посмотреть pitch[t] и rhythm[t] на удерживаемых нотах. Зафиксировать константы с комментарием-ссылкой на конкретные индексы pkl |
...
| 6 | Семантика `chord[0]` vs `chord[max_len]` в CMT (какой фрейм — стартовый, какой — реальный) определяется в `chord_forward` (model.py:119-120: `chord_out[:, 1:, ...]` пропускает первый). Нужно подтвердить на реальных pkl-файлах какие индексы заполнены данными | В Task 6 (chroma converter) первый шаг — diagnostic: открыть `seed_instance.pkl`, посмотреть `chord.toarray()[0]` и `chord.toarray()[-1]`. Адаптировать формулу разворачивания соответственно (нули в `[0]` vs нули в `[max_len]`) |
